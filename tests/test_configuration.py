...
def _unset_required_config(cwas_config):
    random_config_key = random.choice(
        ["GENE_MATRIX", "ANNOTATION_DATA_DIR", "SIMULATION_DATA_DIR", "VEP"]
    )
...
def _set_invalid_file_path(cwas_config, invalid_file_path):
    random_file_key = random.choice(
        ["GENE_MATRIX", "ANNOTATION_KEY_CONFIG", "BIGWIG_CUTOFF_CONFIG", "SIMULATION_PATHS"]
    )
...
def _check_config_outputs(cwas_workspace):
    ...
    sim_data_dir_symlink = cwas_workspace / "simulation-data"
    ...
    sim_filepaths_symlink = cwas_workspace / "simulation_filepaths.yaml"

    assert data_dir_symlink.is_dir() and data_dir_symlink.is_symlink()
    assert sim_data_dir_symlink.is_dir() and sim_data_dir_symlink.is_symlink()
    ...
    assert sim_filepaths_symlink.is_file() and sim_filepaths_symlink.is_symlink()

    # Teardown
    data_dir_symlink.unlink()
    sim_data_dir_symlink.unlink()
