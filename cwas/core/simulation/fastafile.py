...
class FastaFile:
    def __init__(self, fasta_file_path: Path):
        """
        :param fasta_file_path: This file path should end with
                                '.fa' or '.fa.gz'.
        """
        if not ''.join(fasta_file_path.suffixes) == '.fa.gz' \
                and not fasta_file_path.suffix == '.fa':
